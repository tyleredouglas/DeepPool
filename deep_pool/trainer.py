...
def train(model, train_loader, val_loader,
    ...
    for epoch in range(start_epoch, n_epochs+1):
        ...
        # concatenate & drop any non-finite pairs before calculated r^2
...
def main():
    ap = argparse.ArgumentParser(
        ...
        train error predictor for adaptive windowing. 
    ...
    ap.add_argument("--snp-freqs-csv",  required=True,
                    help="csv of SNP frequencies from training populations")
    ap.add_argument("--features-csv",   required=True,
                    help="features table from haplomatic-window")
    ap.add_argument("--feature-list",   required=True,
                    help="which features to include from haplomatic-window")
    ap.add_argument("--hap-names-file", required=True)
    ap.add_argument("--region",         default=None,
                    help="optionally train individual modules for each specified region")
    ap.add_argument("--max-snps",       type=int,   default=400,
                    help="max number of SNPs per window (right-cropped)")
