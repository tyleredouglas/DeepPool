...
class ReadSimulator:
    """
    Simulate paired-end reads from a population DataFrame where index is
    (chrom, pos) and columns contain founder IDs.
    """
    ...
    def generate_reads(
        ...
        """
        Generates n paired-end reads, writes:
        ...
        with open(f"{out_prefix}_1.fastq", "w") as fq1, \
            ...
            while read_cnt < n_reads:
                ...
                if isinstance(hap, pd.Series):  
                    hap = hap.iloc[0]

                # increment depth at the SNP (use .loc for MultiIndex)
                key = (contig, nearest_pos)
                hap_counts.loc[key, hap] = hap_counts.loc[key, hap] + 1
