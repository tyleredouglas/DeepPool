...
def main() -> None:
    p = argparse.ArgumentParser(
        ...
        description=textwrap.dedent("""\
            simulates populations and generates paired-end read data from them.
    ...
    p.add_argument("--ril-df",
                   help="CSV of genotyped RILs (CHROM,pos,RIL1..)")
    p.add_argument("--n-rils",     type=int,   default=300,
                   help="if ril-df not provided, number of RILs to simulate to seed initial populations")
    p.add_argument("--step-bp",    type=int,   default=10_000,
                   help="genotype step in simulated RILs")
    p.add_argument("--mean-block", type=int,   default=100_000,
                   help="average haplotype block size (i.e. linkage) in simulated RILs")
    p.add_argument("--k-active",   type=int,   default=3,
                   help="how many haplotypes co-occur at each location in simulated RIL panel")
    p.add_argument("--alpha-base", type=float, default=0.25,
                   help="how balanced should mixing prorportions of haplotypes in simulated RILs")
    ...
    p.add_argument("--threads", "--n-workers",
                   ...
                   help="threads for parallel read‐generation")
